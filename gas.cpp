...
void Flatulan::move()
{
    ...
    if(dir==UP){
        if(m_row>1 && m_city->player()->row()!=m_row-1 && m_city->player()->col()!=m_col) m_row--;
    }else if(dir==DOWN){
        if(m_row<m_city->rows() && m_city->player()->row()!=m_row+1 && m_city->player()->col()!=m_col) m_row++;
    }else if(dir==LEFT){
        if(m_col>1 && m_city->player()->row()!=m_row && m_city->player()->col()!=m_col-1) m_col--;
    }else if (dir==RIGHT){
        if(m_col<m_city->cols() && m_city->player()->row()!=m_row && m_city->player()->col()!=m_col+1) m_col++;
    }
}
...
void Player::move(int dir)
{
    ...
    int r = m_row;
    int c = m_col;
    if(dir==UP && m_city->nFlatulansAt(r-1, c)==0){
        if(m_row>1) m_row--;
    }else if(dir==DOWN &&  m_city->nFlatulansAt(r+1, c)==0){
        if(m_row<m_city->rows()) m_row++;
    }else if(dir==LEFT &&  m_city->nFlatulansAt(r, c-1)==0){
        if(m_col>1) m_col--;
    }else if (dir==RIGHT &&  m_city->nFlatulansAt(r, c+1)==0){
        if(m_col<m_city->cols()) m_col++;
    }
}
...
int City::nFlatulansAt(int r, int c) const
{
    int count = 0;
    for(int i = 0; i < m_nFlatulans; i++){
        if(m_flatulans[i]->row() == r && m_flatulans[i]->col()==c) count++;
    }
    return count;
}
...
void City::display() const
{
    ...
    //        For 9 or more, set it to '9'. CHECK
    for (r = 1; r <= rows(); r++){
        for (c = 1; c <= cols(); c++){
            if(nFlatulansAt(r,c)==1){
                grid[r-1][c-1] = 'F';
            }else if(nFlatulansAt(r, c)>1){
                grid[r-1][c-1] = 48 + nFlatulansAt(r, c);
            }
        }
    }
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    for(int r = m_player->row()-1; r <= m_player->row()+1; r++){
        for(int c = m_player->col()-1; c<=m_player->col()+1; c++){
            if(!(r==m_player->row()) || !(c==m_player->col())){
                int num = nFlatulansAt(r, c);
                for(int n = 0; n<num; n++){
                    bool temp = m_flatulans[0]->possiblyGetConverted();
                    for(int i = 0; i<m_nFlatulans; i++){
                        if(temp && m_flatulans[i]->row() == r && m_flatulans[i]->col() == c){
                            delete m_flatulans[i];
...
void City::moveFlatulans()
{
    ...
    {
        m_flatulans[k]->move();
        for(int r = m_flatulans[k]->row()-1; r <= m_flatulans[k]->row()+1; r++){
            for(int c = m_flatulans[k]->col()-1; c<=m_flatulans[k]->col()+1; c++){
                if(r==m_player->row() && c==m_player->col()){
                    m_player->getGassed();
                }
            }
        ...
        //        adjacent to the player, the player suffers a gas blast. CHECK
    }
}
